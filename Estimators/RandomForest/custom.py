...
"""
# This custom estimator task implements a Random forest classifier
...
def fit(X, y, output_dir, class_order, row_weights, **kwargs):
    ...
    output_dir: str
        A path to the output folder; the artifact [in this example - containing the trained RandomForestClassifier] must be saved into this folder.
    class_order: list
    ...
    estimator.set_params({'criterion': 'gini', 'max_features': 'auto', 'min_samples_split': 2, 'n_estimators': 100, 'random_state': 0})
    estimator.fit(X, y)

    # dump the trained object [in this example - a trained RandomForestClassifier]
...
def score(data, model, **kwargs):
    ...
    model: Any
        ...
        In this example, contains trained DecisionTreeClassifier extracted from artifact.pkl.
    Returns
    -------
    pd.DataFrame
